...
async function getStats(userId) {
  ...
  let aggregatorOpts = [
    ...
    { $match: { "subscribedUsers.user": mongoose.Types.ObjectId(userId) } },
    {
      $group: {
        _id: "$type",
        count: { $sum: 1 }
  ...
  let stats = await Event.aggregate(aggregatorOpts).exec();
...
async function getStatsIn(userId, startDate, endDate) {
  let events = await Event.find({ "subscribedUsers.user": userId })
    .find({
      datetime: {
